...
local function openMenu(data, sort, skipFirst)
	...
	for _,v in pairs(data) do
		if v["icon"] then
			...
			if exports.ox_inventory:Items(tostring(v['icon'])) then
				v["icon"] = img..exports.ox_inventory:Items(tostring(v['icon'])).name .. '.png'
                print(v["icon"])
			end
		end
	end
        ...
        if data then
            if data.params.event then
                ...
                elseif data.params.isQBCommand then
                    ...
                    print('Fitur ini tidak tersedia di Esx')
